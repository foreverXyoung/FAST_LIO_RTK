...
class ImuProcess
{
 public:
  ...
  void set_rtk_init(const RtkData &rtk_pose, bool enable);
 ...
 private:
  ...
  /** ==================== RTK Extension (START) ==================== **/
  bool   rtk_need_init_ = true;     // RTK初始化是否待执行
  RtkData init_rtk_pose_;           // 用于初始化的RTK位姿
  bool   use_rtk_init_ = false;     // 是否启用RTK初始化
  /** ==================== RTK Extension (END) ==================== **/
};
...
void ImuProcess::set_rtk_init(const RtkData &rtk_pose, bool enable)
{
  // 保存RTK位姿, 在IMU初始化完成后用于覆盖EKF的初始pos/rot/grav/vel
  init_rtk_pose_ = rtk_pose;
  use_rtk_init_ = enable;
  rtk_need_init_ = true;
}
